...
function submitPoint() {
  ...
  if (editPointForm.id) {
    skyNetApi.updatePoint(editPointForm.id, data).then(() => {
      ...
      loadPoints()
      eventBus.emit('dataUpdated', 'skyNetPoint', 'update', editPointForm.id)
    }).catch((err) => {
    ...
    skyNetApi.createPoint(data).then(() => {
      ...
      showDialog.value = false
      loadPoints()
      eventBus.emit('dataUpdated', 'skyNetPoint', 'create')
    }).catch((err) => {
...
function deletePoint(id: number) {
  ElMessageBox.confirm('确定删除该点位？', '提示', {
    ...
    skyNetApi.deletePoint(id).then(() => {
      ...
      loadPoints()
      eventBus.emit('dataUpdated', 'skyNetPoint', 'delete', id)
    }).catch((err) => {
