...
const pagedCheckpoints = computed(() => {
  ...
  return checkpoints.value.slice(start, start + perPage.value)
})
...
function submitDevice() {
  ...
  if (editForm.id) {
    checkpointPointApi.updateCheckpoint(pointId, editForm.id, data).then(() => {
      ...
      loadData()
      eventBus.emit('dataUpdated', 'checkpoint', 'update', editForm.id)
    }).catch((err) => {
    ...
    checkpointPointApi.createCheckpoint(pointId, data).then(() => {
      ...
      showAddDialog.value = false
      loadData()
      eventBus.emit('dataUpdated', 'checkpoint', 'create')
    }).catch((err) => {
...
function deleteDevice(id: number) {
  ...
  ElMessageBox.confirm('确定删除该设备？', '提示', {
    ...
    checkpointPointApi.deleteCheckpoint(pointId, id).then(() => {
      ...
      loadData()
      eventBus.emit('dataUpdated', 'checkpoint', 'delete', id)
    }).catch((err) => {
  ...
  Promise.all([
    ...
    if (checkpoints.value.length > 0) {
      if (point.value?.selected_project_id) {
        const savedItem = checkpoints.value.find(item => item.id === point.value?.selected_project_id)
        if (savedItem) {
          selectedProject.value = savedItem.id
        } else {
          let defaultItem = checkpoints.value[0]
...
function submitExtendWarranty() {
  ...
  checkpointPointApi.extendWarranty(pointId, {
    ...
    loadData()
    eventBus.emit('dataUpdated', 'checkpoint', 'update')
  }).catch((err) => {
...
watch(selectedProject, async (newVal) => {
  if (newVal && point.value && !loading.value) {
    try {
      await checkpointPointApi.update(Number(route.params.id), {
